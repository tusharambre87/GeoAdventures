...
import { plannerPlaces, plannerStopIntelligence } from "@shared/schema";
...
export async function enrichStop(
  ...
  if (cachedRow) {
    console.log(`[StopEnrichment] Cache hit for: ${stop.name} (canonical: ${canonicalPlaceId})`);
    ...
    return { ...enriched, placeId };
  }

  console.log(`[StopEnrichment] Enriching: ${stop.name} (placeId: ${placeId})`);
  const profile = await callEnrichmentAI(stop);
