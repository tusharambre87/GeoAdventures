...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Waitlist form endpoint - sends email to support with registration info
  ...
  app.get('/api/users', isAuthenticated, isAdmin, async (req: any, res) => {
    try {
      ...
      const { db } = await import("./db");
      const { users, players } = await import("@shared/schema");
  ...
  // Builder preview — returns real planner_places data for a city, or empty array (frontend falls back to static)
  app.get('/api/travel/builder-preview', async (req: any, res) => {
    try {
      const cityRaw = (req.query.city as string || "").trim();
      const childAgesRaw = (req.query.childAges as string || "");
      if (!cityRaw) return res.json({ spots: [] });

      const childAges = childAgesRaw
        ? childAgesRaw.split(",").map(Number).filter(n => !isNaN(n))
        : [];
      const minAge = childAges.length > 0 ? Math.min(...childAges) : 0;
      const maxAge = childAges.length > 0 ? Math.max(...childAges) : 18;
      ...
      const rows = await db
        .select({
          ...
          type: plannerPlaces.type,
          familyAnchorType: plannerPlaces.familyAnchorType,
          minAge: plannerPlaces.minAge,
          maxAge: plannerPlaces.maxAge,
          whyNow: plannerPlaces.whyNow,
          effortLevel: plannerPlaces.effortLevel,
          indoorOutdoor: plannerPlaces.indoorOutdoor,
          whyItWorks: plannerPlaceProfiles.whyItWorks,
        })
        .from(plannerPlaces)
        .leftJoin(plannerPlaceProfiles, eq(plannerPlaceProfiles.placeId, plannerPlaces.id))
        .where(ilike(plannerPlaces.city, `%${cityLower}%`))
        .orderBy(
          drizzleSql`CASE WHEN ${plannerPlaces.familyAnchorType} = 'anchor' THEN 0 WHEN ${plannerPlaces.familyAnchorType} = 'support' THEN 1 ELSE 2 END`
        )
        .limit(20);
      ...
      const ageFiltered = rows.filter(r =>
        (r.maxAge ?? 18) >= minAge && (r.minAge ?? 0) <= maxAge + 4
      );
      const spots = (ageFiltered.length >= 3 ? ageFiltered : rows).slice(0, 4).map(r => ({
        emoji: ANCHOR_EMOJIS[r.type] || "⭐",
        name: r.name,
        reason: r.whyItWorks || r.whyNow || `A great ${r.type} stop for families`,
        anchorType: r.familyAnchorType,
      }));

      res.json({ spots });
    } catch (err) {
      console.error("[BuilderPreview] Error:", err);
      res.json({ spots: [] });
    }
  });
          ...
          for (const extraCity of remainingCities) {
            ...
            (async () => {
              try {
                ...
                const generatedStops = await generateCityStops(capturedCity, null, capturedCountry, cityStopCount, capturedStyle || 'family_explorer', undefined, capturedAnchors);
                for (let i = 0; i < generatedStops.length; i++) {
                  const s = generatedStops[i];
                  await storage.createStop({
                    ...
                    stopMissions: s.stopMissions
                      ? s.stopMissions.map((m: any) => ({ ...m, xpReward: m.type === 'photo' ? 10 : 5, completed: false, skipped: false, attempts: 0 }))
                      : null,
                    cityGroup: capturedCity,
                  });
                }
                console.log(`✅ [Travel] [bg-multicity] Generated ${generatedStops.length} stops for "${capturedCity}" in trip ${capturedTripId}`);
              } catch (err) {
  ...
  app.post('/api/travel/trips/:tripId/generate-city-stops', isAuthenticated, travelModeGuard, async (req: any, res) => {
    try {
      ...
      const generatedStops = await generateCityStops(cityName, state || null, country || '', stopCount, adventureStyle || 'family_explorer', undefined, anchorsForGeneration);
      ...
      for (let i = 0; i < generatedStops.length; i++) {
        ...
        const created = await storage.createStop({
          ...
          missionDifficulty: stop.missionDifficulty || 'normal',
          missionKeepsakeReward: stop.missionKeepsakeReward || false,
          stopMissions: stop.stopMissions ? stop.stopMissions.map(m => ({ ...m, xpReward: m.type === 'photo' ? 10 : 5, completed: false, skipped: false, attempts: 0 })) : null,
      ...
      console.log(`✅ [Travel] Generated ${createdStops.length} stops for "${cityName}" in trip ${tripId}`);
...
Return ONLY valid JSON in this exact format:
{
  ...
  app.get('/api/travel/trips/:tripId/wallet', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const { tripWalletItems, travelTrips } = await import("@shared/schema");
      const { eq } = await import("drizzle-orm");
  ...
  app.post('/api/travel/trips/:tripId/wallet', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const userId = req.user?.claims?.sub;
      if (!userId) return res.status(401).json({ message: "Unauthorized" });
      const { tripWalletItems, travelTrips, insertTripWalletItemSchema } = await import("@shared/schema");
      const { eq } = await import("drizzle-orm");
      const [trip] = await db.select({ id: travelTrips.id, userId: travelTrips.userId }).from(travelTrips).where(eq(travelTrips.id, tripId));
      if (!trip) return res.status(404).json({ message: "Trip not found" });
      if (trip.userId !== userId) return res.status(403).json({ message: "Access denied" });
...
Format your response as JSON:
{
  ...
  app.get('/api/travel/trophies/:explorerId', isAuthenticated, travelModeGuard, async (req: any, res) => {
    try {
      ...
      const { TRAVEL_BADGE_CATEGORIES } = await import('@shared/schema');
