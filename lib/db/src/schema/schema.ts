...
export const plannerTripPlanStops = pgTable("planner_trip_plan_stops", {
   ...
   *  exceeds paceConfig.totalStopMinutes.max for this family's pace. */
  budgetExceeded: boolean("budget_exceeded").default(false),
  /** Human-readable budget overage summary, e.g.
   *  "day total 280 min exceeds Balanced budget of 240 min". */
...
// ============================================================================
// STOP LIBRARY — Persistent per-stop cache seeded from city pools + AI output
...
export const stopLibrary = pgTable("stop_library", {
  ...
  latitude: varchar("latitude"),
  longitude: varchar("longitude"),
  stopType: varchar("stop_type").notNull().default("landmark"),
  description: text("description"),
  stopMissions: jsonb("stop_missions"),
  keepsake: jsonb("keepsake"),
  storyPack: jsonb("story_pack"),
  audioUrl: text("audio_url"),
  enrichment: jsonb("enrichment"),
  source: varchar("source").notNull().default("ai_generated"), // canonical|seeded|ai_generated
  ...
  distanceFromMetroKm: numeric("distance_from_metro_km", { precision: 6, scale: 1 }), // haversine km from city center
  imageUrl: text("image_url"),               // hero image stored in object storage (stop-images/...)
}, (table) => [
