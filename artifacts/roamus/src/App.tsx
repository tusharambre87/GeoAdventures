...
const pageImports = {
  ...
  AppDownload: () => import("@/pages/AppDownload"),
};
...
const AppDownload = lazyWithRetry(pageImports.AppDownload);
...
function isAppDownloadDomain(): boolean {
  const hostname = window.location.hostname;
  return hostname === 'app.roamus.app';
}
...
function Router() {
  ...
  const isAppDomain = isAppDownloadDomain();
  
  const getRootComponent = () => {
    if (isAppDomain) return AppDownload;
    if (isGamesSubdomain) return GeoGamesLanding;
    return GeoAdventuresLanding;
  };
