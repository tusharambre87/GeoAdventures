    ...
    <View style={[s.root, { backgroundColor: G.bg }]}>
      ...
      <View style={[s.header, { paddingTop: insets.top + 14 }]}>
        <View style={s.navRow}>
          ...
          <Text style={s.title}>Travel crew</Text>
          <View style={{ width: 40 }} />
        </View>
        <Text style={s.sub}>Add or remove people for your trips.</Text>
      </View>
      ...
      {loadingInitial ? (
        ...
        >
              ...
              return (
                <View key={t.id} style={s.bubbleWrap}>
                  <View style={[s.bubble, { backgroundColor: bg, shadowColor: bg }]}>
                    ...
                    {i > 0 && (
                      <Pressable onPress={() => remove(t.id)} style={s.removeBtn} hitSlop={4}>
            ...
            <View style={s.form}>
              ...
              <View style={[s.formInput, { borderColor: newName ? "rgba(232,105,42,0.4)" : "rgba(26,31,46,0.1)" }]}>
                <TextInput
                  ...
                  placeholder={addingType === "child" ? "e.g. Aaron" : "e.g. Partner"}
                  ...
                  <View style={s.ageGrid}>
                    {AGES.map(a => (
                      <Pressable
                        ...
                        onPress={() => setNewAge(a)}
                        style={[s.ageChip, {
                          backgroundColor: newAge === a ? G.orange : "rgba(26,31,46,0.06)",
                          borderColor: newAge === a ? G.orange : "transparent",
                        }]}
                      >
                        <Text style={{
                          fontFamily: newAge === a ? F.bold : F.regular,
                          fontSize: 14,
                          fontWeight: newAge === a ? "700" : "400",
                          color: newAge === a ? "#fff" : G.deep,
                        }}>{a}</Text>
                      </Pressable>
                    ))}
                  </View>
              ...
              <View style={s.formActions}>
                <Pressable
                  onPress={() => { setAddingType(null); setNewName(""); setNewAge(8); }}
                  style={s.cancelBtn}
                >
                ...
                <Pressable
                  ...
                  style={[s.confirmBtn, { backgroundColor: newName.trim() ? G.orange : "rgba(232,105,42,0.3)" }]}
                >
                  <Text style={{ fontFamily: F.bold, fontSize: 15, fontWeight: "700", color: "#fff" }}>
                    Add {newName || addingType}
                  </Text>
                </Pressable>
              </View>
            </View>
          ...
          {youngest && (
            <View style={s.note}>
              <Text style={s.noteTitle}>{'\u2713'} Trips adapt to your family</Text>
              <Text style={s.noteBody}>
                {(youngest.age ?? 0) <= 4
                  ? `With a ${youngest.age}-year-old, stops stay under 45 min with rest breaks.`
                  : (youngest.age ?? 0) <= 7
                  ? "We mix hands-on experiences with landmarks — right for ages 5–7."
                  : "Old enough for deep exploration, missions, and real discovery."}
              </Text>
              {hasU5 && (
                <Text style={s.noteExtra}>{'\uD83D\uDEBC'} Stroller-friendly stops prioritised</Text>
              )}
            </View>
          )}
        </ScrollView>
...
const s = StyleSheet.create({
  ...
  backArrow:    { fontFamily: F.regular, fontSize: 22, color: G.deep },
  title:        { flex: 1, fontFamily: F.bold, fontSize: 22, fontWeight: "800", color: G.deep, textAlign: "center", letterSpacing: -0.4 },
  ...
  cta:          { position: "absolute", left: 0, right: 0, bottom: 0, paddingHorizontal: 24, paddingTop: 14, backgroundColor: G.bg },
  saveBtn:      { backgroundColor: G.orange, borderRadius: 16, paddingVertical: 16, alignItems: "center" },
  saveBtnDisabled: { opacity: 0.5 },
  saveBtnText:  { fontFamily: F.bold, fontSize: 16, fontWeight: "700", color: "#fff" },
});
