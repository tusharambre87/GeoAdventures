...
function AvatarCircle({
  ...
  const r = size / 2;
  const base = { width: size, height: size, borderRadius: r, alignItems: "center" as const, justifyContent: "center" as const, overflow: "hidden" as const };
  if (adultStyle) {
    return (
      <View style={[base, { backgroundColor: "#2C3E6B" }]}>
        <Text style={{ fontSize: size * 0.38 }}>{"\uD83D\uDC64"}</Text>
      </View>
    );
  }
  if (isUrl(avatar)) {
    return <Image source={{ uri: avatar! }} style={[base, { borderWidth: 2, borderColor: "#E8692A" }]} contentFit="cover" />;
  }
  if (isEmoji(avatar)) {
    return (
      <View style={[base, { backgroundColor: "#FDF0E9" }]}>
  ...
  return (
    <View style={[base, { backgroundColor: FALLBACK_COLORS[idx] }]}>
...
function EmojiPickerPanel({
  ...
  return (
    <View style={ep.panel}>
      ...
      <View style={ep.grid}>
        {AVATAR_EMOJIS.map(e => (
          <Pressable key={e} onPress={() => onSelect(e)} style={[ep.circle, selected === e && ep.circleSelected]}>
...
export default function TravelersScreen() {
  ...
  const [editingAvatarForId, setEditingAvatarForId] = useState<number | string | null>(null);
  ...
  useEffect(() => {
    apiFetch<{ travelers?: any[] }>("/api/users/travelers")
      ...
      .catch(() => setTravelers([{ id: 0, init: "Y", name: "You", isParent: true }]))
  ...
  async function pickPhotoAndUpload(): Promise<string | null> {
    ...
    try {
      const up = await fetch(`${API_BASE}/api/users/avatar-upload`, {
        method: "POST",
  ...
  async function handlePickPhotoForExisting(id: number | string) {
    const url = await pickPhotoAndUpload();
    if (url) {
