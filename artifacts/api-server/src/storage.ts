...
export interface IStorage {
  ...
  // ============================================================================
  // STOP LIBRARY — Persistent per-stop cache
  // ============================================================================
  getStopLibraryByCity(city: string, country: string): Promise<StopLibrary[]>;
  saveStopLibraryEntries(entries: InsertStopLibrary[]): Promise<StopLibrary[]>;
  updateStopLibraryServeStats(ids: string[]): Promise<void>;
  getStopLibraryStatus(): Promise<{ totalStops: number; enrichedStops: number; citiesCovered: number; byCity: { city: string; country: string; count: number; enriched: number }[]; byCountry: { country: string; cities: number; stops: number; enriched: number; pending: number }[] }>;
  getUnenrichedStopLibraryIds(limit?: number, country?: string): Promise<string[]>;
  getStopLibraryByNames(pairs: { city: string; name: string }[]): Promise<StopLibrary[]>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getStopLibraryByCity(city: string, country: string): Promise<StopLibrary[]> {
    const normalizedKey = `${city.toLowerCase().trim()}:${country.toLowerCase().trim()}`;
    return db
      .select()
      .from(stopLibrary)
      .where(eq(stopLibrary.normalizedKey, normalizedKey))
  ...
  async updateStopLibraryServeStats(ids: string[]): Promise<void> {
    if (ids.length === 0) return;
    await db
      .update(stopLibrary)
      .set({ serveCount: sql`${stopLibrary.serveCount} + 1`, lastServedAt: new Date() })
      .where(inArray(stopLibrary.id, ids));
  }

  async getStopLibraryStatus(): Promise<{ totalStops: number; enrichedStops: number; citiesCovered: number; byCity: { city: string; country: string; count: number; enriched: number }[]; byCountry: { country: string; cities: number; stops: number; enriched: number }[] }> {
    const [totals] = await db.select({
      totalStops: sql<number>`count(*)`,
      enrichedStops: sql<number>`count(case when ${stopLibrary.storyPack} is not null then 1 end)`,
      citiesCovered: sql<number>`count(distinct ${stopLibrary.normalizedKey})`,
    }).from(stopLibrary);
    ...
    const byCity = await db.select({
      ...
      count: sql<number>`count(*)`,
      enriched: sql<number>`count(case when ${stopLibrary.storyPack} is not null then 1 end)`,
    }).from(stopLibrary)
      .groupBy(stopLibrary.city, stopLibrary.country)
      .orderBy(desc(sql`count(*)`));

    const byCountry = await db.select({
      country: stopLibrary.country,
      cities: sql<number>`count(distinct ${stopLibrary.normalizedKey})`,
      stops: sql<number>`count(*)`,
      enriched: sql<number>`count(case when ${stopLibrary.storyPack} is not null then 1 end)`,
      pending: sql<number>`count(case when ${stopLibrary.storyPack} is null then 1 end)`,
    }).from(stopLibrary)
      .groupBy(stopLibrary.country)
      .orderBy(desc(sql`count(*)`));

    return {
      totalStops: Number(totals?.totalStops ?? 0),
      enrichedStops: Number(totals?.enrichedStops ?? 0),
      citiesCovered: Number(totals?.citiesCovered ?? 0),
      byCity: byCity.map(r => ({ city: r.city, country: r.country, count: Number(r.count), enriched: Number(r.enriched) })),
      byCountry: byCountry.map(r => ({ country: r.country, cities: Number(r.cities), stops: Number(r.stops), enriched: Number(r.enriched), pending: Number(r.pending) })),
    };
  }

  async getUnenrichedStopLibraryIds(limit = 100, country?: string): Promise<string[]> {
    const baseWhere = isNull(stopLibrary.storyPack);
    const rows = await db
      .select({ id: stopLibrary.id })
      .from(stopLibrary)
      .where(country ? and(baseWhere, eq(stopLibrary.country, country)) : baseWhere)
      .limit(limit);
    return rows.map(r => r.id);
  }

  async getStopLibraryByNames(pairs: { city: string; name: string }[]): Promise<StopLibrary[]> {
    if (pairs.length === 0) return [];
    // Match by lowercase(city) and name — avoids needing country on travel_stops rows.
    // Uses DB-side lower() on city column and an IN filter on name for efficiency.
    const cities = [...new Set(pairs.map(p => p.city.toLowerCase().trim()))];
    const names = pairs.map(p => p.name);
    return db
      .select()
      .from(stopLibrary)
      .where(and(
        inArray(sql`lower(${stopLibrary.city})`, cities),
        inArray(stopLibrary.name, names)
      ));
  }
